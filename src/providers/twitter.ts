import { BuildUrlParams, SocialProvider } from '../social-share-urls';
import UrlError from '../url-error';

const name = 'Twitter';

const Twitter: SocialProvider = {
	name,

	link: ({ url, text, via, hashtags }: BuildUrlParams) => {
		if (!url) {
			throw new UrlError(name);
		}
